...
ollama pull qwen2.5-coder:3b                    # ~2 GB — summaries
ollama pull nomic-embed-text                    # ~275 MB — meaning-based search and Ask
```

**Two models, two jobs** — one writes text, the other turns text into vectors, and neither comes
with the other. After pulling the second one, build the index once under `/bot` → **Meaning index**;
until then search by meaning and `/ask` find nothing. The card there says whether the model is
