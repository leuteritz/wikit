...
if (fs.existsSync(dist)) {
  ...
  app.get('/', (req, res) =>
    res.type('text/plain').send('Frontend noch nicht gebaut. Bitte "npm run build" ausfuehren (oder im Dev "npm run dev").')
  )
}

app.listen(PORT, HOST, () => {
  console.log(`Wikit laeuft auf http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}`)
})
