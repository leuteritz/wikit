...
<template>
  <div class="space-y-5">
    <!-- ======================= Vorgemerkt ======================= -->
    <section class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]">
      <header class="flex flex-wrap items-center gap-2 border-b border-[var(--color-border)] px-4 py-2.5">
        <Icon icon="lucide:git-fork" class="h-4 w-4 text-[var(--color-accent)]" />
        <h3 class="text-sm font-semibold text-[var(--color-text)]">Staged changes</h3>
        <span class="rounded bg-[var(--color-surface-offset)] px-1.5 font-mono text-2xs text-[var(--color-text-muted)]">
          {{ changes.length }} / {{ max }}
        </span>
        <span v-if="running" class="inline-flex items-center gap-1 text-2xs text-[var(--color-text-muted)]">
        ...
        <div class="ml-auto flex items-center gap-2">
          <button
            ...
            type="button"
            class="inline-flex items-center gap-1 rounded-md border border-[var(--color-border)] px-2 py-1 text-2xs text-[var(--color-text-muted)] transition hover:border-[var(--color-border-strong)] hover:text-[var(--color-text)]"
            @click="clear"
          >
      ...
      <ul v-if="changes.length" class="divide-y divide-[var(--color-border)]">
        <li v-for="(c, i) in changes" :key="i" class="flex items-start gap-3 px-4 py-2.5">
          ...
          <div class="min-w-0 flex-1">
            <p
              ...
              :class="rowFor(i) && !rowFor(i).applied ? 'text-[var(--color-text-muted)] line-through' : 'text-[var(--color-text)]'"
            >
              {{ rowFor(i)?.title || localTitle(c) }}
            </p>
            <p v-if="rowFor(i) && !rowFor(i).applied" class="mt-0.5 text-2xs text-[var(--color-text-muted)]">
              No effect — {{ rowFor(i).reason }}
            </p>
            <p v-else-if="rowFor(i)?.detail" class="mt-0.5 text-2xs text-[var(--color-text-muted)]">
              {{ rowFor(i).detail }}
            </p>
            <p v-if="rowFor(i)?.warning" class="mt-0.5 inline-flex items-start gap-1 text-2xs text-[var(--color-warning)]">
          ...
          <button
            ...
            type="button"
            class="shrink-0 rounded p-1 text-[var(--color-text-muted)] transition hover:bg-[var(--color-surface-offset)] hover:text-[var(--color-text)]"
            @click="removeAt(i)"
          >
      ...
      <div v-else class="px-4 py-5 text-center">
        <p class="text-xs text-[var(--color-text-muted)]">
          ...
          <span class="font-medium text-[var(--color-text)]">Cycles</span> and
          <span class="font-medium text-[var(--color-text)]">Rules</span> tabs, where every finding
    ...
    <!-- ======================= Etwas vormerken ======================= -->
    <section class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-4 py-3">
      <div class="flex flex-wrap items-center gap-2">
        <Icon icon="lucide:search" class="h-3.5 w-3.5 text-[var(--color-text-muted)]" />
        <input
          v-model="term"
          type="text"
        ...
        <button
          v-if="picked || term"
          type="button"
          class="rounded p-1 text-[var(--color-text-muted)] transition hover:text-[var(--color-text)]"
          @click="reset"
        >
          <Icon icon="lucide:x" class="h-3.5 w-3.5" />
        </button>
      ...
      <ul v-if="matches.length" class="mt-2 space-y-0.5 border-t border-[var(--color-border)] pt-2">
        <li v-for="c in matches" :key="c.id">
          <button
            type="button"
            class="flex w-full items-baseline gap-2 rounded px-2 py-1 text-left transition hover:bg-[var(--color-surface-offset)]"
            @click="choose(c)"
          >
            <span class="font-mono text-xs text-[var(--color-text)]">{{ c.className }}</span>
            <span class="truncate font-mono text-3xs text-[var(--color-text-muted)]">{{ c.package || '(default)' }}</span>
          </button>
        </li>
      </ul>
      ...
      <!-- Ist eine Klasse gewählt, stehen hier ihre möglichen Umbauten – und zwar alle vier
           ...
           beantworten soll. -->
      <div v-if="picked" class="mt-3 space-y-3 border-t border-[var(--color-border)] pt-3">
        <p v-if="full" class="text-2xs text-[var(--color-warning)]">
          That is as many changes as one simulation carries ({{ max }}). Remove one first.
        </p>

        <!-- Beziehungen -->
        <div>
          <p class="mb-1.5 text-2xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
            Its relations
          </p>
          <BusyState v-if="!edgesReady" variant="inline" title="Loading relations…" :since="startedAt" />
          <p v-else-if="!relations.length" class="text-2xs text-[var(--color-text-muted)]">
          ...
          <ul v-else class="max-h-48 space-y-0.5 overflow-y-auto">
            <li
              ...
              class="flex items-center gap-2 rounded px-2 py-1 hover:bg-[var(--color-surface-offset)]"
            >
              <span class="min-w-0 flex-1 truncate font-mono text-xs text-[var(--color-text)]">
                {{ nameOf(r.from) }}
                <Icon icon="lucide:arrow-right" class="mx-0.5 inline h-3 w-3 text-[var(--color-text-muted)]" />
                {{ nameOf(r.to) }}
              </span>
              <button
                v-tip="'Simulate removing this dependency'"
                type="button"
                class="inline-flex shrink-0 items-center gap-1 rounded border border-[var(--color-border)] px-1.5 py-0.5 text-3xs text-[var(--color-text-muted)] transition hover:border-[var(--color-border-strong)] hover:text-[var(--color-text)] disabled:opacity-40"
                :disabled="full"
        ...
        <div class="grid gap-3 sm:grid-cols-3">
          <div>
            ...
            <div class="flex items-center gap-1.5">
              <select
                v-model="movePkg"
                class="min-w-0 flex-1 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-1 font-mono text-3xs text-[var(--color-text)]"
              >
              ...
              <button
                type="button"
                class="shrink-0 rounded-md border border-[var(--color-border)] px-2 py-1 text-3xs text-[var(--color-text-muted)] transition hover:text-[var(--color-text)] disabled:opacity-40"
          ...
          <div>
            ...
            <input
              ...
              class="w-full rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-1 font-mono text-3xs text-[var(--color-text)] outline-none"
            />
            <ul v-if="mergeMatches.length" class="mt-1 space-y-0.5">
              <li v-for="m in mergeMatches.slice(0, 4)" :key="m.id">
                <button
                  type="button"
                  class="w-full truncate rounded px-1.5 py-0.5 text-left font-mono text-3xs text-[var(--color-text-muted)] transition hover:bg-[var(--color-surface-offset)] hover:text-[var(--color-text)] disabled:opacity-40"
                  :disabled="full"
          ...
          <div>
            ...
            <button
              type="button"
              class="inline-flex items-center gap-1.5 rounded-md border border-[var(--color-border)] px-2 py-1 text-3xs text-[var(--color-text-muted)] transition hover:border-[var(--color-danger)]/50 hover:text-[var(--color-danger)] disabled:opacity-40"
              :disabled="full"
    ...
    <!-- ======================= Die Bilanz ======================= -->
    <p v-if="error" class="rounded-lg border border-[var(--color-danger)]/40 bg-[var(--color-danger)]/10 px-4 py-2.5 text-xs text-[var(--color-text)]">
      {{ error }}
    </p>

    <div
      v-else-if="result && !result.available"
      class="rounded-lg border border-[var(--color-warning)]/40 bg-[var(--color-warning)]/10 px-4 py-3 text-xs text-[var(--color-text)]"
    >
      <p class="font-medium">
    ...
    <template v-else-if="totals">
      <section class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]">
        <header class="flex items-center gap-2 border-b border-[var(--color-border)] px-4 py-2.5">
          <Icon icon="lucide:scale" class="h-4 w-4 text-[var(--color-accent)]" />
          <h3 class="text-sm font-semibold text-[var(--color-text)]">What it would change</h3>
          <button
            v-tip="'Copy the whole plan — changes, effect and cost'"
            type="button"
            class="ml-auto inline-flex items-center gap-1.5 rounded-md border border-[var(--color-border)] px-2.5 py-1 text-2xs text-[var(--color-text-muted)] transition hover:border-[var(--color-border-strong)] hover:text-[var(--color-text)]"
            :disabled="copying"
            @click="copyPlan"
          >
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[30rem] text-xs">
            <thead>
              ...
              <tr class="border-b border-[var(--color-border)] text-2xs uppercase tracking-wide text-[var(--color-text-muted)]">
            ...
            <tbody class="divide-y divide-[var(--color-border)]">
              <tr v-for="r in rows" :key="r.key" :class="r.moved ? '' : 'text-[var(--color-text-muted)]'">
                <td class="px-4 py-1.5" :class="r.lead && r.moved ? 'font-semibold text-[var(--color-text)]' : ''">
                  {{ r.label }}
                  <span v-if="r.named && r.nameAfter" class="ml-1 font-mono text-3xs text-[var(--color-text-muted)]">
                ...
                <td class="px-3 py-1.5 text-right font-mono tabular-nums" :class="r.moved ? 'text-[var(--color-text)]' : ''">
                  {{ fmt(r.after, r.decimal) }}
                </td>
                <td class="px-4 py-1.5 text-right font-mono tabular-nums">
                  <span
                    v-if="r.moved"
                    :class="r.good === true ? 'text-[var(--color-success)]' : r.good === false ? 'text-[var(--color-danger)]' : 'text-[var(--color-text)]'"
                  >
        ...
        <p v-if="quiet" class="flex items-center gap-2 border-t border-[var(--color-border)] px-4 py-2.5 text-xs text-[var(--color-text-muted)]">
          <Icon icon="lucide:check-circle" class="h-3.5 w-3.5 text-[var(--color-success)]" />
        ...
        <p v-if="cost" class="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-[var(--color-border)] px-4 py-2.5 text-2xs text-[var(--color-text-muted)]">
      ...
      >
        <h3 class="flex items-center gap-2 text-sm font-semibold text-[var(--color-text)]">
          <Icon icon="lucide:scale" class="h-4 w-4" :class="newlyBroken.length ? 'text-[var(--color-danger)]' : 'text-[var(--color-success)]'" />
        ...
        <ul class="mt-2 space-y-2">
          <li v-for="r in ruleChanges" :key="r.line" class="text-xs">
            <p class="font-mono text-[var(--color-text)]">{{ r.text }}</p>
            <p class="mt-0.5 text-2xs text-[var(--color-text-muted)]">
            ...
            <p
              ...
              :key="v.fromId + '-' + v.toId"
              class="mt-0.5 font-mono text-3xs text-[var(--color-text-muted)]"
            >
              {{ v.from }} → {{ v.to }}
            </p>
          </li>
      ...
      <section v-if="cycles" class="grid gap-4 lg:grid-cols-2">
        <div class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-4 py-3">
          <h3 class="flex items-center gap-2 text-sm font-semibold text-[var(--color-text)]">
            <Icon icon="lucide:check-circle" class="h-4 w-4 text-[var(--color-success)]" />
            Loops that would be gone
            <span class="font-mono text-2xs text-[var(--color-text-muted)]">
              {{ cycles.classes.healed.length + cycles.packages.healed.length }}
            </span>
          </h3>
          <p v-if="!cycles.classes.healed.length && !cycles.packages.healed.length" class="mt-1.5 text-2xs text-[var(--color-text-muted)]">
            None — these changes do not open any loop.
          </p>
          <ul v-else class="mt-2 space-y-1.5">
            <li v-for="(c, i) in cycles.classes.healed" :key="'c' + i" class="font-mono text-2xs text-[var(--color-text-muted)]">
              {{ c.chain.join(' → ') }}
            </li>
            <li v-for="(c, i) in cycles.packages.healed" :key="'p' + i" class="font-mono text-2xs text-[var(--color-text-muted)]">
              <span class="mr-1 rounded bg-[var(--color-surface-offset)] px-1 text-3xs">pkg</span>
              {{ c.chain.join(' → ') }}
            </li>
          </ul>
        </div>
        ...
        <div class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-4 py-3">
          <h3 class="flex items-center gap-2 text-sm font-semibold text-[var(--color-text)]">
            <Icon icon="lucide:repeat" class="h-4 w-4" :class="cycles.classes.appeared.length ? 'text-[var(--color-danger)]' : 'text-[var(--color-text-muted)]'" />
            Loops it would create
            <span class="font-mono text-2xs text-[var(--color-text-muted)]">
              {{ cycles.classes.appeared.length + cycles.packages.appeared.length }}
            </span>
          </h3>
          <p v-if="!cycles.classes.appeared.length && !cycles.packages.appeared.length" class="mt-1.5 text-2xs text-[var(--color-text-muted)]">
            None. Nothing here closes a new circle.
          </p>
          <ul v-else class="mt-2 space-y-2">
            <li v-for="(c, i) in cycles.classes.appeared" :key="'c' + i" class="text-2xs">
              <p class="font-mono text-[var(--color-text)]">{{ c.chain.join(' → ') }}</p>
              <p v-for="(e, j) in c.closedBy" :key="j" class="mt-0.5 font-mono text-3xs text-[var(--color-warning)]">
                closed by {{ e.from }} → {{ e.to }} ({{ e.kind }})
              </p>
            </li>
            <li v-for="(c, i) in cycles.packages.appeared" :key="'p' + i" class="font-mono text-2xs text-[var(--color-text)]">
              <span class="mr-1 rounded bg-[var(--color-surface-offset)] px-1 text-3xs">pkg</span>
              {{ c.chain.join(' → ') }}
            </li>
          </ul>
      ...
      <section
        ...
        class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] px-4 py-3"
      >
        <h3 class="flex items-center gap-2 text-sm font-semibold text-[var(--color-text)]">
          <Icon icon="lucide:flame" class="h-4 w-4 text-[var(--color-text-muted)]" />
        ...
        <ul class="mt-2 divide-y divide-[var(--color-border)]">
          <li v-for="c in result.classes.shifted" :key="c.id" class="flex items-center gap-3 py-1.5 text-xs">
            <button
              type="button"
              class="min-w-0 flex-1 truncate text-left font-mono text-[var(--color-text)] transition hover:text-[var(--color-accent)]"
            ...
            <span
              ...
              :class="c.delta < 0 ? 'text-[var(--color-success)]' : c.delta > 0 ? 'text-[var(--color-warning)]' : 'text-[var(--color-text-muted)]'"
            >
              {{ c.score.before }} → {{ c.score.after }}
            </span>
          </li>
          <li v-for="c in result.classes.gone" :key="'g' + c.id" class="flex items-center gap-3 py-1.5 text-xs">
            <span class="min-w-0 flex-1 truncate font-mono text-[var(--color-text-muted)] line-through">{{ c.className }}</span>
            <span class="shrink-0 text-2xs text-[var(--color-text-muted)]">gone — {{ plural(c.loc, 'line') }}</span>
          </li>
        </ul>
        <p v-if="result.classes.moreShifted" class="mt-2 text-2xs text-[var(--color-text-muted)]">
      ...
      <div class="rounded-lg border border-[var(--color-border)] px-4 py-3 text-2xs leading-relaxed text-[var(--color-text-muted)]">
        <p class="font-semibold uppercase tracking-wide">What this does not know</p>
        <ul class="mt-1.5 space-y-1">
          <li>
            <span class="font-medium text-[var(--color-text)]">Nothing here is written.</span>
          ...
          <li>
            <span class="font-medium text-[var(--color-text)]">It moves relations, not code.</span>
          ...
          <li>
            <span class="font-medium text-[var(--color-text)]">A score is a rank inside its own state.</span>
