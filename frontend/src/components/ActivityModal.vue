...
<template>
  <!-- Escape schliesst hier NICHT selbst: das macht App.vue zusammen mit Palette und
       Kuerzel-Uebersicht – ein Druck darf nicht zwei Overlays gleichzeitig wegnehmen. -->
  <Modal :open="detailOpen" size="lg" max-height="max-h-[85vh]" label="Activity" @close="closeDetail">
    ...
    <header class="flex shrink-0 items-center gap-3 border-b border-line px-5 py-3.5">
      <span
        ...
        :class="busy ? 'bg-accent-soft text-accent' : 'bg-surface-offset text-muted'"
      >
        <Icon :icon="headIcon" class="h-[18px] w-[18px]" :class="run ? 'animate-pulse' : ''" />
      </span>
      <div class="min-w-0 flex-1">
        <h2 class="truncate font-mono text-[0.9375rem] font-semibold tracking-tight text-ink">{{ headTitle }}</h2>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto">
      ...
      <div v-if="queue" class="border-t border-line px-5 py-4" :class="run ? '' : 'border-t-0'">
        <div class="mb-2 flex items-center gap-2">
          <Icon icon="lucide:sparkles" class="h-4 w-4 shrink-0 text-lavender" />
          <span class="font-mono text-2xs font-semibold uppercase tracking-[0.12em] text-muted">AI analysis</span>
          <span class="ml-auto font-mono text-sm font-semibold tabular-nums text-lavender">{{ queue.percent }}%</span>
        </div>
        <div class="h-2 w-full overflow-hidden rounded-full bg-surface-offset">
          <div
            class="h-full rounded-full bg-lavender transition-[width] duration-500 ease-out"
            :style="{ width: queue.percent + '%' }"
          />
        </div>
        <p class="mt-2.5 truncate text-sm font-medium text-ink">{{ queueDetail }}</p>
        <div class="mt-3 grid grid-cols-3 gap-2">
          <div class="rounded-xl border border-line bg-surface px-3 py-2 text-center">
            <div class="font-mono text-base font-semibold tabular-nums text-ink">{{ nf.format(queue.done) }}<span class="text-muted">/{{ nf.format(queue.total) }}</span></div>
            <div class="mt-0.5 text-3xs font-medium uppercase tracking-wide text-muted">steps</div>
          </div>
          <div class="rounded-xl border border-line bg-surface px-3 py-2 text-center">
            <div class="font-mono text-base font-semibold tabular-nums text-ink">{{ formatEta(queue.etaMs) || '–' }}</div>
            <div class="mt-0.5 text-3xs font-medium uppercase tracking-wide text-muted">remaining</div>
          </div>
          <div class="rounded-xl border border-line bg-surface px-3 py-2 text-center">
            <div class="font-mono text-base font-semibold tabular-nums text-ink">{{ queue.classesLeft }}</div>
            <div class="mt-0.5 text-3xs font-medium uppercase tracking-wide text-muted">classes left</div>
          </div>
        </div>
        <!-- Die Parallelitaet steht nur da, wenn sie vom Normalfall abweicht: bei 1 waere sie
             ...
             Schritte × Dauer. -->
        <p v-if="queue.parallel > 1" class="mt-2 text-2xs text-muted">
      ...
      <div v-if="!busy" class="px-5 py-6">
        <div class="grid grid-cols-3 gap-2">
          <div class="rounded-xl border border-line bg-surface px-3 py-3 text-center">
            <div class="font-mono text-xl font-semibold tabular-nums text-ink">{{ nf.format(stock.classes) }}</div>
            <div class="mt-0.5 text-3xs font-medium uppercase tracking-wide text-muted">classes</div>
          </div>
          <div class="rounded-xl border border-line bg-surface px-3 py-3 text-center">
            <div class="font-mono text-xl font-semibold tabular-nums text-ink">{{ nf.format(stock.packages) }}</div>
            <div class="mt-0.5 text-3xs font-medium uppercase tracking-wide text-muted">packages</div>
          </div>
          <div class="rounded-xl border border-line bg-surface px-3 py-3 text-center">
            <div class="font-mono text-xl font-semibold tabular-nums text-ink">{{ nf.format(stock.analyzed) }}</div>
            <div class="mt-0.5 text-3xs font-medium uppercase tracking-wide text-muted">summarized</div>
          </div>
        </div>
        ...
        >
          ...
          />
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-ink">{{ lastRun.message }}</p>
            <p class="mt-0.5 flex flex-wrap items-center gap-x-1.5 font-mono text-2xs text-muted">
...
/* Die eigene `act-modal`-Transition ist entfallen: sie war eine von vier fast gleichen Fassungen
   derselben Geste und steht jetzt einmal in `ui/Modal.vue`. */
</style>
