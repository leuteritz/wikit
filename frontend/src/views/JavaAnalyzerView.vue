...
<template>
  <div class="flex h-[calc(100vh-3.5rem)] flex-col px-5 py-6">
    <div class="mb-4 shrink-0">
      <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Java Analyzer</h1>
      <p class="text-sm text-slate-500 dark:text-slate-400">
        Java-Code analysieren – lokal geparst, Abhängigkeiten als Graph, KI-Zusammenfassung pro Methode.
      </p>
    </div>

    <!-- Tabs -->
    <div class="mb-4 flex shrink-0 gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-800/60">
      <button
        type="button"
        class="rounded-md px-4 py-1.5 text-sm font-medium transition"
        :class="tab === 'upload' ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-700 dark:text-indigo-300' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'"
        @click="tab = 'upload'"
      >Hochladen</button>
      <button
        type="button"
        class="rounded-md px-4 py-1.5 text-sm font-medium transition"
        :class="tab === 'graph' ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-700 dark:text-indigo-300' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'"
        @click="tab = 'graph'"
      >Graph-Ansicht</button>
    </div>
    ...
    <div v-show="tab === 'upload'" class="min-h-0 flex-1 overflow-y-auto">
      <div class="mx-auto max-w-3xl">
        ...
        <div class="mt-3 flex items-center gap-3">
          ...
          <span class="text-xs text-slate-400">Wird lokal geparst – kein Cloud-Dienst.</span>
        </div>
      </div>
    </div>

    <!-- Graph-Tab -->
    <div v-show="tab === 'graph'" class="relative flex min-h-0 flex-1 gap-3">
      <div class="min-w-0 flex-1">
        <JavaDependencyGraph :reload-token="reloadToken" @select="onSelect" />
      </div>
      <Transition name="panel">
        <div v-if="selectedFileId" class="h-full w-full max-w-md shrink-0">
          <JavaClassDetail :file-id="selectedFileId" @close="onDetailClose" />
        </div>
      </Transition>
    </div>
  </div>
...
<style scoped>
.panel-enter-active, .panel-leave-active { transition: opacity 0.15s ease; }
